...
const hdrBtn: React.CSSProperties = {
  ...
  color: "white", borderRadius: "9px", padding: "7px 14px", cursor: "pointer", fontSize: "13px", fontWeight: "600",
};

// ── AudioRecorder inline (corrigé Safari + Brave) ──
function AudioRecorder({ onTranscription, compact = false }: { onTranscription: (text: string, keywords: string[]) => void; compact?: boolean }) {
  ...
  const start = () => {
    setError("");
    // Safari: webkitSpeechRecognition / Chrome: SpeechRecognition / Brave: peut bloquer
    const SR = (window as any).SpeechRecognition || (window as any).webkitSpeechRecognition;
    if (!SR) {
      setError("⚠️ Reconnaissance vocale non supportée. Utilisez Safari ou Chrome.");
      return;
    }
    try {
      const r = new SR();
      ...
      r.onerror = (e: any) => {
        if (e.error === "not-allowed") setError("⚠️ Microphone bloqué. Autorisez l'accès dans les paramètres du navigateur.");
        else if (e.error === "service-not-allowed") setError("⚠️ Brave bloque la reconnaissance vocale. Utilisez Safari ou désactivez Brave Shields.");
...
export default function Visit() {
  ...
  const fetchAll = async () => {
    try {
      const [forms, boit, tract, reun, even] = await Promise.all([
        ...
        fetch(`${API}/api/evenements`).then(r => r.json()),
      ]);
  ...
  const saveAudio = async (transcription: string, keywords: string[]) => {
    ...
    try {
      await fetch(`${API}/api/audio`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ form_id: lastFormId, transcription, mots_cles: keywords }) });
      setAudioSaved(true);
  ...
  const submitBoitage = async () => {
    ...
    setBSubmitting(true);
    try {
      await fetch(`${API}/api/boitage`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...bForm, nom_benevole: agentName, date: today }) });
  ...
  const submitTractage = async () => {
    ...
    setTSubmitting(true);
    try {
      await fetch(`${API}/api/tractage`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...tForm, nom_benevole: agentName, date: today }) });
  ...
  const submitReunion = async () => {
    ...
    setRSubmitting(true);
    try {
      await fetch(`${API}/api/reunions`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...rForm, agent_nom: agentName }) });
    ...
    <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: C.white, borderTop: `1px solid ${C.border}`, display: "flex", zIndex: 100, boxShadow: "0 -2px 10px rgba(0,0,0,0.08)" }}>
      {navItems.map(n => (
        <button key={n.key} onClick={() => { setView(n.key); setBShow(false); setTShow(false); setRShow(false); setShowAudio(false); }}
          ...
          <span style={{ fontSize: "9px", fontWeight: "700", color: view === n.key ? n.color : C.muted, textTransform: "uppercase", letterSpacing: "0.3px" }}>{n.label}</span>
  ...
  if (view === "dashboard") return (
    <div style={pageStyle}>
      ...
      <div style={content}>
        ...
        {/* AUJOURD'HUI : stats 2x2 gauche + carte événement droite */}
          ...
          {evenements.length > 0 && (() => {
            ...
            return (
              <div onClick={() => setView("evenement")} style={{ width: "130px", flexShrink: 0, background: C.white, borderRadius: "14px", border: `2px solid ${C.g1}`, padding: "10px", cursor: "pointer", boxShadow: "0 2px 8px rgba(0,0,0,0.08)" }}>
                {e.photo_url
                  ? <img src={e.photo_url} alt="" style={{ width: "100%", height: "70px", objectFit: "cover", borderRadius: "8px", marginBottom: "8px" }} />
                ...
                {e.sujets && <div style={{ fontSize: "10px", color: C.text, textAlign: "center", marginTop: "4px", borderTop: `1px solid ${C.border}`, paddingTop: "4px" }}>{e.sujets}</div>}
              </div>
            );
          })()}
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "12px" }}>
          {[
            ...
            <button key={a.key} onClick={() => setView(a.key)}
              style={{ background: C.white, border: `2px solid ${C.border}`, borderRadius: "14px", padding: "16px", cursor: "pointer", textAlign: "left", boxShadow: "0 2px 8px rgba(0,0,0,0.05)", transition: "all 0.15s" }}
        ...
        <div style={{ display: "flex", justifyContent: "center", marginBottom: "24px" }}>
          <button onClick={() => setView("evenement")} style={{ background: C.white, border: `1.5px solid ${C.border}`, borderRadius: "12px", padding: "10px 40px", cursor: "pointer", color: C.text, fontWeight: "600", fontSize: "13px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
            📅 Événement
          </button>
        </div>
  ...
  if (view === "visite") return (
    <div style={pageStyle}>
      ...
      <div style={content}>
        {!showAudio ? (
          <QuizForm agentName={agentName} onBack={() => setView("dashboard")} onSubmit={(d) => { handleFormSubmit(d); showSuccess("Visite enregistrée ✅"); }} />
        ) : (
          <div>
  ...
  if (view === "boitage") return (
    <div style={pageStyle}>
      ...
      <div style={content}>
        ...
        {!bShow ? (
          <button onClick={() => setBShow(true)} style={{ width: "100%", padding: "15px", background: GRADIENT, color: "white", border: "none", borderRadius: "12px", cursor: "pointer", fontWeight: "700", fontSize: "15px", marginBottom: "16px", boxShadow: "0 4px 14px rgba(26,58,107,0.3)" }}>
            ➕ Nouveau boîtage
          </button>
        ) : (
          <div style={card}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                <div><label style={lbl}>Nb tracts</label>
                  <input type="number" min="0" style={inp} placeholder="0" value={bForm.nb_tracts} onChange={e => setBForm({ ...bForm, nb_tracts: e.target.value })} />
                </div>
                <div><label style={lbl}>Nb boîtes</label>
                  <input type="number" min="0" style={inp} placeholder="0" value={bForm.nb_boites} onChange={e => setBForm({ ...bForm, nb_boites: e.target.value })} />
                </div>
              </div>
              <div><label style={lbl}>Commentaire</label>
                <textarea style={{ ...inp, resize: "vertical", minHeight: "70px" }} value={bForm.commentaire} onChange={e => setBForm({ ...bForm, commentaire: e.target.value })} placeholder="Notes..." />
              </div>
        ...
        {boitages.length === 0 ? (
          ...
          <div key={b.id} style={card}>
            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "6px" }}>
              ...
              {b.date === today && <span style={{ background: "#fef9e7", color: "#7d5a00", padding: "1px 6px", borderRadius: "6px", fontSize: "10px", fontWeight: "700" }}>Aujourd'hui</span>}
            </div>
            ...
            {b.commentaire && <p style={{ margin: "6px 0 0", fontSize: "12px", color: C.muted }}>{b.commentaire}</p>}
          </div>
        ))}
      </div>
      <BottomNav />
  ...
  if (view === "tractage") return (
    <div style={pageStyle}>
      ...
      <div style={content}>
        ...
        {!tShow ? (
          <button onClick={() => setTShow(true)} style={{ width: "100%", padding: "15px", background: `linear-gradient(135deg, #c97a00, #e67e22)`, color: "white", border: "none", borderRadius: "12px", cursor: "pointer", fontWeight: "700", fontSize: "15px", marginBottom: "16px", boxShadow: "0 4px 14px rgba(201,122,0,0.3)" }}>
            ➕ Nouveau tractage
          </button>
        ) : (
          <div style={card}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              ...
              <div><label style={lbl}>Adresse exacte</label>
                <input style={inp} placeholder="Ex: Avenue d'Italie" value={tForm.adresse} onChange={e => setTForm({ ...tForm, adresse: e.target.value })} />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                <div><label style={lbl}>Nb tracts donnés</label>
                  <input type="number" min="0" style={inp} placeholder="0" value={tForm.nb_tracts} onChange={e => setTForm({ ...tForm, nb_tracts: e.target.value })} />
                </div>
                <div><label style={lbl}>Nb portes frappées</label>
                  <input type="number" min="0" style={inp} placeholder="0" value={tForm.nb_portes} onChange={e => setTForm({ ...tForm, nb_portes: e.target.value })} />
                </div>
              </div>
              <div><label style={lbl}>Commentaire</label>
                <textarea style={{ ...inp, resize: "vertical", minHeight: "70px" }} value={tForm.commentaire} onChange={e => setTForm({ ...tForm, commentaire: e.target.value })} placeholder="Notes..." />
              </div>
              <div style={{ display: "flex", gap: "8px" }}>
        ...
        {tractages.length === 0 ? (
          ...
          <div key={t.id} style={card}>
            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "6px" }}>
              ...
              {t.date === today && <span style={{ background: "#fef9e7", color: "#7d5a00", padding: "1px 6px", borderRadius: "6px", fontSize: "10px", fontWeight: "700" }}>Aujourd'hui</span>}
            </div>
            ...
            {t.commentaire && <p style={{ margin: "6px 0 0", fontSize: "12px", color: C.muted }}>{t.commentaire}</p>}
          </div>
        ))}
      </div>
      <BottomNav />
      ...
      <div style={content}>
        ...
        {!rShow ? (
          <button onClick={() => { setRShow(true); setRStep("form"); }}
            style={{ width: "100%", padding: "15px", background: `linear-gradient(135deg, ${C.g3}, ${C.g1})`, color: "white", border: "none", borderRadius: "12px", cursor: "pointer", fontWeight: "700", fontSize: "15px", marginBottom: "16px", boxShadow: "0 4px 14px rgba(26,122,107,0.3)" }}>
            ➕ Nouvelle réunion
          </button>
        ) : (
          <div style={card}>
            ...
            {rStep === "form" && (
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                <div><label style={lbl}>Titre *</label><input style={inp} placeholder="Ex: Réunion de quartier Tolbiac" value={rForm.titre} onChange={e => setRForm({ ...rForm, titre: e.target.value })} /></div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                  ...
                  <div><label style={lbl}>Lieu</label><input style={inp} placeholder="Salle, adresse..." value={rForm.lieu} onChange={e => setRForm({ ...rForm, lieu: e.target.value })} /></div>
                </div>
                <div><label style={lbl}>Ordre du jour</label><textarea style={{ ...inp, resize: "vertical", minHeight: "70px" }} placeholder="Points à aborder..." value={rForm.ordre_du_jour} onChange={e => setRForm({ ...rForm, ordre_du_jour: e.target.value })} /></div>
                <div><label style={lbl}>Participants</label><input style={inp} placeholder="Jean D., Marie L., ..." value={rForm.participants} onChange={e => setRForm({ ...rForm, participants: e.target.value })} /></div>
            ...
            {rStep === "audio" && (
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                <p style={{ fontSize: "13px", color: C.muted, margin: "0 0 8px" }}>Enregistre et transcris l'audio de la réunion :</p>
                <AudioRecorder onTranscription={(text) => { setRForm(prev => ({ ...prev, transcription: text })); showSuccess("Transcription enregistrée ✅"); }} />
                {rForm.transcription && (
                  <div style={{ background: C.light, borderRadius: "10px", padding: "12px", fontSize: "13px", color: C.text, maxHeight: "120px", overflowY: "auto" }}>
                    <strong>Transcription :</strong> {rForm.transcription}
                  </div>
                )}
            ...
            {rStep === "rapport" && (
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                <div><label style={lbl}>Rapport de réunion</label>
                  <textarea style={{ ...inp, resize: "vertical", minHeight: "120px" }} placeholder="Résumé, décisions prises, prochaines étapes..." value={rForm.rapport} onChange={e => setRForm({ ...rForm, rapport: e.target.value })} />
                </div>
                ...
                <div style={{ display: "flex", gap: "8px" }}>
                  <button onClick={submitReunion} disabled={rSubmitting} style={{ flex: 2, padding: "12px", background: rSubmitting ? "#aaa" : `linear-gradient(135deg, ${C.g3}, ${C.g1})`, color: "white", border: "none", borderRadius: "10px", cursor: "pointer", fontWeight: "700" }}>{rSubmitting ? "Envoi..." : "✅ Enregistrer la réunion"}</button>
        ...
        {reunions.length === 0 ? (
          ...
          <div key={r.id} style={card}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "6px" }}>
              <div style={{ fontWeight: "700", fontSize: "14px", color: C.text }}>{r.titre}</div>
              <span style={{ fontSize: "11px", color: C.muted, flexShrink: 0, marginLeft: "8px" }}>{r.date}</span>
            </div>
            {r.lieu && <div style={{ fontSize: "12px", color: C.muted, marginBottom: "5px" }}>📍 {r.lieu}</div>}
            {r.participants && <div style={{ fontSize: "12px", color: C.muted, marginBottom: "5px" }}>👥 {r.participants}</div>}
            <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
            ...
            {r.rapport && <div style={{ background: C.light, borderRadius: "8px", padding: "10px", marginTop: "8px", fontSize: "12px", color: C.text, lineHeight: "1.5" }}>{r.rapport.slice(0, 200)}{r.rapport.length > 200 ? "..." : ""}</div>}
          </div>
        ))}
      </div>
      <BottomNav />
