...
export default function QuizForm({ agentName, onBack, onSubmit }: QuizFormProps) {
  ...
  const label: React.CSSProperties = { fontSize: "11px", fontWeight: "700", color: C.muted, textTransform: "uppercase", letterSpacing: "0.4px", display: "block", marginBottom: "5px" };

  const RadioCard = ({ value, current, field, label: lbl }: { value: string; current: string; field: keyof QuizFormData; label: string }) => (
  ...
  const CheckCard = ({ value, checked, onChange, label: lbl }: { value: string; checked: boolean; onChange: () => void; label: string }) => (
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', sans-serif" }}>
      ...
      {step === 1 && (
        <div>
          ...
          <div style={card}>
            ...
            <span style={label}>Quartier *</span>
          ...
          <div style={card}>
            ...
            {[["maison", "🏠 Maison"], ["immeuble", "🏢 Immeuble"], ["batiment", "🏗 Bâtiment"]].map(([field, lbl]) => (
              <CheckCard key={field} value={field} checked={(form as any)[field]} onChange={() => setForm({ ...form, [field]: !(form as any)[field] })} label={lbl} />
            ))}
            {(form.immeuble || form.batiment) && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", marginTop: "4px" }}>
                <div>
                  <span style={label}>Étage</span>
                  <input style={input} placeholder="Ex: 3" value={form.etage} onChange={e => setForm({ ...form, etage: e.target.value })} />
                </div>
                <div>
                  <span style={label}>Appartement</span>
                  <input style={input} placeholder="Ex: 12B" value={form.appartement} onChange={e => setForm({ ...form, appartement: e.target.value })} />
                </div>
              </div>
            )}
            {form.batiment && (
              <div style={{ marginTop: "8px" }}>
                <span style={label}>Numéro du bâtiment</span>
      ...
      {step === 2 && (
        <div>
          <div style={card}>
            <span style={label}>Inscrit(e) sur les listes électorales ?</span>
            {["Oui", "Non", "Je ne sais pas"].map(o => <RadioCard key={o} value={o} current={form.inscrit_listes} field="inscrit_listes" label={o} />)}
          </div>
          <div style={card}>
            <span style={label}>Intention de vote aux municipales ?</span>
      ...
      {step === 3 && (
        <div>
          <div style={card}>
            <span style={label}>Qu'est-ce qui vous freine le plus ?</span>
            ...
            {form.raisons_non_vote.includes("Autre") && (
              <div style={{ marginTop: "8px" }}>
                <span style={label}>Précisez</span>
      ...
      {step === 4 && (
        <div>
          <div style={card}>
            <span style={label}>Préoccupations principales</span>
            ...
            <div style={{ marginTop: "8px" }}>
              <span style={label}>Autres préoccupations</span>
      ...
      {step === 5 && (
        <div>
          <div style={card}>
            <span style={label}>Thèmes les plus importants</span>
            ...
            <div style={{ marginTop: "8px" }}>
              <span style={label}>Vos propositions</span>
      ...
      {step === 6 && (
        <div>
          <div style={card}>
            <span style={label}>Souhait de suivi</span>
          ...
          <div style={card}>
            ...
            {form.contact_accord && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", marginTop: "8px" }}>
                {[["Nom", "nom"], ["Prénom", "prenom"], ["Téléphone", "telephone"], ["Email", "email"]].map(([lbl, field]) => (
                  <div key={field}>
                    <span style={label}>{lbl}</span>
                    <input style={input} placeholder={lbl} value={(form as any)[field]} onChange={e => setForm({ ...form, [field]: e.target.value })} />
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={card}>
            ...
            <div style={{ marginTop: "8px" }}>
              <span style={label}>Autres formes de soutien</span>
